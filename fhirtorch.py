...
def classify_columns(df):
    ...
    for column in df.columns:
        ...
        elif df[column].dtype in [np.int, np.int64, np.float, np.float64]:
        ...
        elif '_code' in column.lower():
            umls_columns.append(column)
        # Check if column is categorical
        else:  # change the ratio as needed
            categorical_columns.append(column)

            

    return boolean_columns, numerical_columns, date_columns, categorical_columns, text_columns, umls_columns
